...
export class PrismaSimulatorRepository implements SimulatorRepository {
  ...
  async load(): Promise<SimulatorDataset> {
    ...
    return {
      ...
      materialMfgMappings: tenant.materialMfgMaps.map(row => ({ externalId: row.externalId, materialCode: row.materialCode, internalPn: row.internalPn ?? undefined, manufacturer: row.manufacturer ?? undefined, mpn: row.mpn, relationType: row.relationType ?? undefined, status: row.status ?? undefined, source: row.source, sourceDocumentNo: row.sourceDocumentNo ?? undefined, sourceRow: row.sourceRow ?? undefined, observedAt: iso(row.observedAt), updatedAt: iso(row.sourceUpdatedAt) })),
  ...
  async save(dataset: SimulatorDataset): Promise<void> {
    ...
    await this.db.$transaction(async tx => {
      ...
      await Promise.all([
        ...
        tx.erpSimMaterialMfgMap.deleteMany({ where: { tenantId: this.tenantId } }),
      ...
      if ((dataset.materialMfgMappings ?? []).length) await tx.erpSimMaterialMfgMap.createMany({ data: (dataset.materialMfgMappings ?? []).map(row => ({ tenantId: this.tenantId, externalId: row.externalId, materialCode: row.materialCode, internalPn: row.internalPn, manufacturer: row.manufacturer, mpn: row.mpn, relationType: row.relationType, status: row.status, source: row.source, sourceDocumentNo: row.sourceDocumentNo, sourceRow: row.sourceRow, observedAt: date(row.observedAt), sourceUpdatedAt: date(row.updatedAt) })) })
  ...
  async queryMaterialMfgMappings(q: PageQuery): Promise<PagedRows<import('../src/lib/providers/erp/types.js').ErpMaterialMfgMapping>> {
    const where = {
      tenantId: this.tenantId,
      ...(q.materialCode
        ? { OR: [
            { materialCode: { equals: q.materialCode, mode: 'insensitive' as const } },
            { internalPn: { equals: q.materialCode, mode: 'insensitive' as const } },
          ] }
        : {}),
      ...(q.updatedSince ? { sourceUpdatedAt: { gte: new Date(q.updatedSince) } } : {}),
    }
    const [rows, total] = await Promise.all([
      this.db.erpSimMaterialMfgMap.findMany({ where, orderBy: { externalId: 'asc' }, ...this.page(q) }),
      this.db.erpSimMaterialMfgMap.count({ where }),
    ])
    return { items: rows.map(row => ({ externalId: row.externalId, materialCode: row.materialCode, internalPn: row.internalPn ?? undefined, manufacturer: row.manufacturer ?? undefined, mpn: row.mpn, relationType: row.relationType ?? undefined, status: row.status ?? undefined, source: row.source, sourceDocumentNo: row.sourceDocumentNo ?? undefined, sourceRow: row.sourceRow ?? undefined, observedAt: iso(row.observedAt), updatedAt: iso(row.sourceUpdatedAt) })), total }
  }

  private readTenant() {
    return this.db.erpSimTenant.findUnique({ where: { id: this.tenantId }, include: { materials: true, inventory: true, excess: true, movements: true, lots: true, materialMfgMaps: true, suppliers: true, customers: true, exchangeRates: true, purchaseOrders: { include: { lines: { orderBy: { lineNo: 'asc' } } }, orderBy: { orderDate: 'asc' } }, workOrders: { include: { consumedLines: true }, orderBy: { woNumber: 'asc' } }, receipts: { include: { lines: { orderBy: { lineNo: 'asc' } } }, orderBy: { receivedAt: 'asc' } }, salesOrders: { include: { lines: { orderBy: { lineNo: 'asc' } } }, orderBy: { soNumber: 'asc' } }, scenario: true, requestLogs: { orderBy: { timestamp: 'desc' }, take: 250 }, auditLogs: { orderBy: { timestamp: 'desc' }, take: 250 }, mappingProfiles: true } })
  }
}
