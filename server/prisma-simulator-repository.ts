...
import type { ErpWorkOrder, ScenarioCode, SimulatorDataset } from '../src/lib/providers/erp/types.js'
...
export class PrismaSimulatorRepository implements SimulatorRepository {
  ...
  private readTenant() {
    return this.db.erpSimTenant.findUnique({ where: { id: this.tenantId }, include: { materials: true, inventory: true, excess: true, suppliers: true, customers: true, exchangeRates: true, purchaseOrders: { include: { lines: { orderBy: { lineNo: 'asc' } } }, orderBy: { orderDate: 'asc' } }, workOrders: { include: { consumedLines: true }, orderBy: { woNumber: 'asc' } }, salesOrders: { include: { lines: { orderBy: { lineNo: 'asc' } } }, orderBy: { soNumber: 'asc' } }, scenario: true, requestLogs: { orderBy: { timestamp: 'desc' }, take: 250 }, auditLogs: { orderBy: { timestamp: 'desc' }, take: 250 }, mappingProfiles: true } })
  }
}
